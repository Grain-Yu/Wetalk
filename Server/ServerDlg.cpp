...
LRESULT WINAPI EncodeCallback(HWND hWnd, LPVIDEOHDR lpVHdr)
{
 ...
 //int a=bmpInfo.bmiHeader.biClrImportant;

//char str1[256];
//itoa(a,str1,10);
 //::MessageBox(NULL,str1 , _T(str1), MB_OKCANCEL );



//char szBuffer[1024];
//DWORD dwID = GetCurrentThreadId();
//sprintf(szBuffer, "Process ID: %d", a);
//MessageBox(hWnd, szBuffer, "Popup Title", MB_OK);
...
BOOL CServerDlg::OnInitDialog()
{
	...
	m_ServerSock.Create(845,SOCK_STREAM,"192.168.1.104");
	m_ServerSock.Listen();
...
void CServerDlg::OnButton1() 
{
	...
	if(m_willchating==1)
	{
		...
		SetDlgItemText(IDC_BUTTON1,"停止语音聊天");
		m_willchating=FALSE;


WAVEHDR *wavehdr=reinterpret_cast<PWAVEHDR>(malloc(sizeof(WAVEHDR)));
...
//录音采样格式
	...
	else
	{
		...
		SetDlgItemText(IDC_BUTTON1,"语音聊天");
		m_willchating=1;
